"""
ChatGPT 批量自动注册工具 (并发版) - Zmail 临时邮箱版
依赖: pip install curl_cffi
功能: 使用 Zmail 临时邮箱，并发自动注册 ChatGPT 账号，自动获取 OTP 验证码
"""
...
def _load_config():
    ...
    config = {
        ...
        "upload_api_url": "",
        "upload_api_token": "",
        "cpa_api_url": "",  # CPA 管理 API 地址
        "cpa_management_key": "",  # CPA 管理密钥
        "auto_upload_to_cpa": False,  # 是否自动上传到 CPA
        "oauth_delay": 3,  # OAuth 前等待时间（秒）
    }
    ...
    # 环境变量优先级更高
    config["zmail_base_url"] = os.environ.get("ZMAIL_BASE_URL", config["zmail_base_url"])
    config["zmail_auth_password"] = os.environ.get("ZMAIL_AUTH_PASSWORD", config["zmail_auth_password"])
    config["zmail_email_domain"] = os.environ.get("ZMAIL_EMAIL_DOMAIN", config["zmail_email_domain"])
    config["proxy"] = os.environ.get("PROXY", config["proxy"])
    
    total_env = os.environ.get("TOTAL_ACCOUNTS")
    if total_env is not None:
        config["total_accounts"] = int(total_env)
    ...
    config["cpa_api_url"] = os.environ.get("CPA_API_URL", config["cpa_api_url"])
    config["cpa_management_key"] = os.environ.get("CPA_MANAGEMENT_KEY", config["cpa_management_key"])
    config["auto_upload_to_cpa"] = os.environ.get("AUTO_UPLOAD_TO_CPA", config["auto_upload_to_cpa"])
...
ZMAIL_BASE_URL = _CONFIG["zmail_base_url"]
ZMAIL_AUTH_PASSWORD = _CONFIG["zmail_auth_password"]
ZMAIL_EMAIL_DOMAIN = _CONFIG["zmail_email_domain"]
...
CPA_API_URL = _CONFIG.get("cpa_api_url", "")
CPA_MANAGEMENT_KEY = _CONFIG.get("cpa_management_key", "")
AUTO_UPLOAD_TO_CPA = _as_bool(_CONFIG.get("auto_upload_to_cpa", False))
OAUTH_DELAY = _CONFIG.get("oauth_delay", 3)
...
if not ZMAIL_BASE_URL:
    print("⚠️ 警告: 未设置 ZMAIL_BASE_URL，请在 config.json 中设置或设置环境变量")
    print("   文件: config.json -> zmail_base_url")
    print("   环境变量: export ZMAIL_BASE_URL='https://your-zmail-host.com'")
...
_CHROME_PROFILES = [
    {
        ...
        "build": 6099, "patch_range": (109, 224),
...
def _save_codex_tokens(email: str, tokens: dict):
    ...
    # 上传到新版 CPA 认证管理系统
    if AUTO_UPLOAD_TO_CPA:
        _upload_auth_to_cpa(token_path)
...
def _get_next_proxy():
    ...
    with _proxy_pool_lock:
        proxy = PROXY_POOL[_proxy_pool_index % len(PROXY_POOL)]
        _proxy_pool_index += 1
        return proxy
...
def _create_zmail_session():
    """创建 Zmail 请求会话（不使用 impersonate）"""
    session = curl_requests.Session()
    headers = {
    ...
    if ZMAIL_AUTH_PASSWORD:
        headers["Authorization"] = f"Bearer {ZMAIL_AUTH_PASSWORD}"
    session.headers.update(headers)
    return session


def _get_zmail_domain():
    """获取 Zmail 邮箱域名"""
    if not ZMAIL_BASE_URL:
        return ZMAIL_EMAIL_DOMAIN or "example.com"
    
    try:
        session = _create_zmail_session()
        res = session.get(
            f"{ZMAIL_BASE_URL.rstrip('/')}/api/config",
            timeout=10
        )
        if res.status_code == 200:
            data = res.json()
            domains = data.get("config", {}).get("emailDomains", [])
            if domains:
                return domains[0]
    except Exception:
        pass
...
def create_temp_email():
    """创建 Zmail 临时邮箱，返回 (email, password, mail_address)"""
    if not ZMAIL_BASE_URL:
        raise Exception("ZMAIL_BASE_URL 未设置，无法创建临时邮箱")
    ...
    api_base = ZMAIL_BASE_URL.rstrip("/")
    session = _create_zmail_session()

    try:
        # 1. 尝试创建指定地址的邮箱
        payload = {"address": email_local}
        res = session.post(
        ...
        address = data.get("mailbox", {}).get("address")
        if not address:
            raise Exception("创建邮箱失败: 未返回地址")

        # 2. 获取完整邮箱地址
        domain = _get_zmail_domain()
        ...
        raise Exception(f"Zmail 创建邮箱失败: {e}")


def _fetch_emails_zmail(mail_address: str):
    """从 Zmail 获取邮件列表"""
    try:
        api_base = ZMAIL_BASE_URL.rstrip("/")
        session = _create_zmail_session()
        ...
        res = session.get(
            f"{api_base}/api/mailboxes/{mail_address}/emails",
            timeout=15
        )
...
def _fetch_email_detail_zmail(mail_id: str):
    """获取 Zmail 单封邮件详情"""
    try:
        api_base = ZMAIL_BASE_URL.rstrip("/")
        session = _create_zmail_session()
...
def wait_for_verification_email(mail_address: str, timeout: int = 120):
    """等待并提取 OpenAI 验证码"""
    start_time = time.time()

    while time.time() - start_time < timeout:
        emails = _fetch_emails_zmail(mail_address)
        if emails and len(emails) > 0:
            ...
            for email in emails[:5]:
                ...
                detail = _fetch_email_detail_zmail(mail_id)
                if detail:
                    email_obj = detail.get("email", {})
                    text_content = email_obj.get("textContent", "")
                    html_content = email_obj.get("htmlContent", "")
                    content = subject + " " + text_content + " " + html_content
...
def _save_account_to_zmail(email: str, password: str, platform: str = "OpenAI"):
    """保存账号到 Zmail 账号管理系统"""
    if not ZMAIL_AUTH_PASSWORD:
        return
    ...
    try:
        ...
        if res.status_code == 200:
            data = res.json()
            if data.get("success"):
                print(f"  [Zmail] 账号已保存到管理系统")
        else:
            print(f"  [Zmail] 保存账号失败: {res.status_code}")
    except Exception as e:
        print(f"  [Zmail] 保存账号异常: {e}")


def _upload_auth_to_cpa(token_json_path: str):
    """上传认证 JSON 文件到 CPA 管理系统"""
    if not AUTO_UPLOAD_TO_CPA or not CPA_API_URL or not CPA_MANAGEMENT_KEY:
        return False
    
    if not os.path.exists(token_json_path):
        with _print_lock:
            print(f"  [CPA] 文件不存在: {token_json_path}")
        return False
    ...
    try:
        ...
        # 构造上传请求
        filename = os.path.basename(token_json_path)
        
        # 使用 CurlMime 创建 multipart
        mp = CurlMime()
        mp.addpart(
            name="file",
            content_type="application/json",
        ...
        # 上传到 CPA
        api_url = CPA_API_URL.rstrip("/")
        upload_endpoint = f"{api_url}/v0/management/auth-files"
        ...
        if resp.status_code in [200, 201]:
            with _print_lock:
                print(f"  [CPA] 认证文件已上传: {filename}")
            
            # 上传成功，删除本地文件
            try:
                os.remove(token_json_path)
                with _print_lock:
                    print(f"  [CPA] 本地文件已删除: {filename}")
            except Exception as e:
                with _print_lock:
                    print(f"  [CPA] 删除本地文件失败: {e}")
            
            return True
        else:
            with _print_lock:
                print(f"  [CPA] 上传失败 ({resp.status_code}): {resp.text[:200]}")
                print(f"  [CPA] 本地文件已保留: {filename}")
            return False
            
    except Exception as e:
        with _print_lock:
            print(f"  [CPA] 上传异常: {e}")
            print(f"  [CPA] 本地文件已保留: {os.path.basename(token_json_path)}")
        return False
...
class ChatGPTRegister:
    ...
    def _create_zmail_session(self):
        """创建 Zmail 请求会话"""
        session = curl_requests.Session()
        headers = {
            "User-Agent": self.ua,
    ...
    def create_temp_email(self):
        ...
        api_base = ZMAIL_BASE_URL.rstrip("/")
        session = self._create_zmail_session()
    ...
    def _fetch_emails_zmail(self, mail_address: str):
        """从 Zmail 获取邮件列表"""
        try:
            api_base = ZMAIL_BASE_URL.rstrip("/")
            session = self._create_zmail_session()
    ...
    def _fetch_email_detail_zmail(self, mail_id: str):
        """获取 Zmail 单封邮件详情"""
        try:
            api_base = ZMAIL_BASE_URL.rstrip("/")
            session = self._create_zmail_session()
    ...
    def wait_for_verification_email(self, mail_address: str, timeout: int = 120):
        """等待并提取 OpenAI 验证码"""
        self._print(f"[OTP] 等待验证码邮件 (最多 {timeout}s)...")
        start_time = time.time()

        while time.time() - start_time < timeout:
            emails = self._fetch_emails_zmail(mail_address)
            if emails and len(emails) > 0:
                # 过滤 OpenAI 邮件
                for email in emails[:5]:
                    from_addr = email.get("fromAddress", "").lower()
                    ...
                    detail = self._fetch_email_detail_zmail(mail_id)
                    if detail:
                        email_obj = detail.get("email", {})
                        text_content = email_obj.get("textContent", "")
                        html_content = email_obj.get("htmlContent", "")
    ...
    def run_register(self, email, password, name, birthdate, mail_address):
        """使用 Zmail 的注册流程"""
        self.visit_homepage()
        ...
        if need_otp:
            # 使用 Zmail 等待验证码
            otp_code = self.wait_for_verification_email(mail_address)
            if not otp_code:
            ...
            if status != 200:
                ...
                otp_code = self.wait_for_verification_email(mail_address, timeout=60)
                if not otp_code:
    ...
    def perform_codex_oauth_login_http(self, email: str, password: str, mail_address: str = None):
        ...
        if need_oauth_otp:
            ...
            if not mail_address:
                self._print("[OAuth] OAuth 阶段需要邮箱 OTP，但未提供 mail_address")
                return None
            ...
            while time.time() < otp_deadline and not otp_success:
                emails = self._fetch_emails_zmail(mail_address) or []
                candidate_codes = []

                for email_item in emails[:12]:
...
def _register_one(idx, total, output_file):
    ...
    try:
        ...
        # 1. 创建 Zmail 临时邮箱
        reg._print("[Zmail] 创建临时邮箱...")
        email, email_pwd, mail_address = reg.create_temp_email()
        tag = email.split("@")[0]
        reg.tag = tag  # 更新 tag
        ...
        with _print_lock:
            ...
            print(f"  邮箱地址: {mail_address}")
            print(f"  姓名: {name} | 生日: {birthdate}")
            print(f"{'='*60}")

        # 2. 执行注册流程
        reg.run_register(email, chatgpt_password, name, birthdate, mail_address)

        # 3. OAuth（可选）
        oauth_ok = True
        if ENABLE_OAUTH:
            # 等待账号状态同步
            reg._print(f"[OAuth] 等待账号状态同步 ({OAUTH_DELAY}s)...")
            time.sleep(OAUTH_DELAY)
            
            # 创建新的 session 用于 OAuth，避免 cookie 冲突
            reg._print("[OAuth] 创建新的 OAuth session...")
            old_session = reg.session
            reg.session = curl_requests.Session(impersonate=reg.impersonate)
            if reg.proxy:
                reg.session.proxies = {"http": reg.proxy, "https": reg.proxy}
            reg.session.headers.update({
                "User-Agent": reg.ua,
                "Accept-Language": random.choice([
                    "en-US,en;q=0.9", "en-US,en;q=0.9,zh-CN;q=0.8",
                    "en,en-US;q=0.9", "en-US,en;q=0.8",
                ]),
                "sec-ch-ua": reg.sec_ch_ua, "sec-ch-ua-mobile": "?0",
                "sec-ch-ua-platform": '"Windows"', "sec-ch-ua-arch": '"x86"',
                "sec-ch-ua-bitness": '"64"',
                "sec-ch-ua-full-version": f'"{reg.chrome_full}"',
                "sec-ch-ua-platform-version": f'"{random.randint(10, 15)}.0.0"',
            })
            reg.session.cookies.set("oai-did", reg.device_id, domain="chatgpt.com")
            reg.session.cookies.set("oai-did", reg.device_id, domain=".auth.openai.com")
            reg.session.cookies.set("oai-did", reg.device_id, domain="auth.openai.com")
            
            reg._print("[OAuth] 开始获取 Codex Token...")
            tokens = reg.perform_codex_oauth_login_http(email, chatgpt_password, mail_address=mail_address)
            ...
            # 恢复原 session
            reg.session = old_session

        # 4. 保存账号到 Zmail（可选）
        _save_account_to_zmail(email, chatgpt_password)

        # 5. 线程安全写入结果
        with _file_lock:
            with open(output_file, "a", encoding="utf-8") as out:
                out.write(f"{email}----{chatgpt_password}----{mail_address}----oauth={'ok' if oauth_ok else 'fail'}\n")
...
def run_batch(total_accounts: int = 0, output_file="registered_accounts.txt",
              max_workers=1):
    """并发批量注册 - Zmail 临时邮箱版
    
    Args:
        total_accounts: 注册数量，0 表示无限注册
        output_file: 输出文件
        max_workers: 并发数
    """

    if not ZMAIL_BASE_URL:
        print("❌ 错误: 未设置 ZMAIL_BASE_URL 环境变量")
        print("   请设置: export ZMAIL_BASE_URL='https://your-zmail-host.com'")
        print("   或: set ZMAIL_BASE_URL=https://your-zmail-host.com (Windows)")
        return

    is_infinite = total_accounts == 0
    display_total = "无限" if is_infinite else str(total_accounts)
    
    print(f"\n{'#'*60}")
    print(f"  ChatGPT 批量自动注册 (Zmail 临时邮箱版)")
    print(f"  注册数量: {display_total} | 并发数: {max_workers}")
    print(f"  Zmail: {ZMAIL_BASE_URL}")
    
    if PROXY_POOL:
        print(f"  代理池: {len(PROXY_POOL)} 个代理")
    elif DEFAULT_PROXY:
        print(f"  代理: {DEFAULT_PROXY}")
    else:
        print(f"  代理: 不使用")
    ...
    if ENABLE_OAUTH:
        ...
        if AUTO_UPLOAD_TO_CPA and CPA_API_URL:
            print(f"  CPA 自动上传: 开启")
            print(f"  CPA API: {CPA_API_URL}")
        else:
            print(f"  CPA 自动上传: 关闭")
    ...
    try:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            while is_infinite or idx <= total_accounts:
                # 提交任务
                future = executor.submit(
                    _register_one, idx, display_total, output_file
                )
                futures[future] = idx
                
                # 等待至少一个任务完成
                if len(futures) >= max_workers or (not is_infinite and idx == total_accounts):
                    ...
                    for future in list(done):
                        ...
                        try:
                            ...
                            fail_count += 1
                            with _print_lock:
                                print(f"[FAIL] 账号 {task_idx} 线程异常: {e}")
                        break
                
                idx += 1
                
                # 无限模式下，每完成 10 个显示统计
                if is_infinite and (success_count + fail_count) % 10 == 0:
                    elapsed = time.time() - start_time
                    total_done = success_count + fail_count
                    avg = elapsed / total_done if total_done else 0
                    print(f"\n{'='*60}")
                    print(f"  进度统计: 已完成 {total_done} 个 | 成功 {success_count} | 失败 {fail_count}")
                    print(f"  耗时: {elapsed:.1f}s | 平均: {avg:.1f}s/个")
                    print(f"{'='*60}\n")
    ...
    print(f"\n{'#'*60}")
    print(f"  注册{'中断' if is_infinite else '完成'}! 耗时 {elapsed:.1f} 秒")
    print(f"  总数: {total_done} | 成功: {success_count} | 失败: {fail_count}")
...
def main():
    print("=" * 60)
    print("  ChatGPT 批量自动注册工具 (Zmail 临时邮箱版)")
    print("=" * 60)

    # 检查 Zmail 配置
    if not ZMAIL_BASE_URL:
        print("\n⚠️  警告: 未设置 ZMAIL_BASE_URL")
        print("   请编辑 config.json 设置 zmail_base_url，或设置环境变量:")
        print("   Windows: set ZMAIL_BASE_URL=https://your-zmail-host.com")
        print("   Linux/Mac: export ZMAIL_BASE_URL='https://your-zmail-host.com'")
